...
@FOR /f "delims=" %%i in ('aws ec2 describe-volumes --query "Volumes[*].[VolumeId]" --filters Name=tag:Name,Values=docker-notebook --region us-east-1 --output text
') do SET EBS_VOLUME_ID=%%i
aws ec2 attach-volume --volume-id %EBS_VOLUME_ID% --instance-id %EC2_INSTANCE_ID% --device /dev/xvdf --region us-east-1
